...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			/**
			 *  TODO  初始化bean工厂  设置属性值 beanfactory的准备工作
			 */
			// Prepare the bean factory for use in this context.
			prepareBeanFactory(beanFactory);

			try {
				//子类覆盖方法做额外处理，此处一般不做任何扩展，但是可以查看web中的代码，有具体实现
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//为beanFactory增加一个默认的propertyEditor,这个主要是对bean的属性等设置一个管理的工具类
	...
	protected void initMessageSource() {
		//获取bean工厂 一般为DefaultListableBeanFactory
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		//首先判断是否已有xml文件定义了id为 messageSource的bean对象
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			//如果有，在beanFactory中取出这个对象
			this.messageSource = beanFactory.getBean(MESSAGE_SOURCE_BEAN_NAME, MessageSource.class);
			// Make MessageSource aware of parent MessageSource.
			//如果父类bean工厂不为空,并且这个bean对象是HierarchicalMessageSource类型
			if (this.parent != null && this.messageSource instanceof HierarchicalMessageSource) {
				//类型强制转换成HierarchicalMessageSource类型
			...
			//如果没有xml文件定义信息资源对象,新建DelegatingMessageSource类作为mesasgeSource的bean
			DelegatingMessageSource dms = new DelegatingMessageSource();
			//给这个bean增加父类消息源
			dms.setParentMessageSource(getInternalParentMessageSource());
			this.messageSource = dms;
			//将这个messageSource实例之策到bean工厂中
	...
	protected void initApplicationEventMulticaster() {
		...
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		//校验bean工厂中是否包含applicationEventMulticaster这个bd,也就是说自定义的时间监听多路广播器
	...
	protected void registerListeners() {
		...
		//首先获取所有的listener,遍历应用程序中存在的监听器集合,并将对应的监听器添加到监听器的多路广播器中
		...
		//从容器中获取所有实现了ApplicationListener接口的bean 的name
		//放入到集合中
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果beanFactory之前没有注入嵌入值解析器,则注册默认的嵌入值解析器,主要用于注解属性值的解析
		//BFPP的时候加入了
		...
		//初始化LoadTimeWeaverAware bean ，以便尽早注册他们的转换器
